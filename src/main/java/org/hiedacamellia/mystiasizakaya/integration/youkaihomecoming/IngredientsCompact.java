...
public class IngredientsCompact {
	public static ItemStack execute(ItemStack itemstack) {
		if ((BuiltInRegistries.ITEM.get(itemstack.getItem()).toString()).equals("youkaihomecoming:raw_lamprey")) {
			return new ItemStack(MIItem.BA_MU_MAN.get());
		}
		if ((BuiltInRegistries.ITEM.get(itemstack.getItem()).toString()).equals("youkaihomecoming:tofu")) {
			return new ItemStack(MIItem.DOU_FU.get());
		}
		if ((BuiltInRegistries.ITEM.get(itemstack.getItem()).toString()).equals("youkaihomecoming:butter")) {
