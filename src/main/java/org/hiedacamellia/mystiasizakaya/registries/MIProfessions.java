...
public class MIProfessions {
	...
	public static final DeferredRegister<VillagerProfession> PROFESSIONS = DeferredRegister.create(ForgeRegistries.VILLAGER_PROFESSIONS, MystiasIzakaya.MODID);
	public static final RegistryObject<VillagerProfession> TRADER = registerProfession("trader", () -> MIBlock.COOKING_RANGE.get(), () -> ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.smoker.smoke")));
		...
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			for (Map.Entry<String, ProfessionPoiType> entry : POI_TYPES.entrySet()) {
				...
				if (existingCheck.isPresent()) {
					Debug.getLogger().error("Skipping villager profession " + name + " that uses POI block " + block + " that is already in use by " + existingCheck);
					continue;
				}
