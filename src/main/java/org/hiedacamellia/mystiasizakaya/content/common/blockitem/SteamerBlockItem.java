...
public class SteamerBlockItem extends KitchenwareBlockItem {
    public SteamerBlockItem() {
        super(MIBlock.STEAMER.get(), new Properties().stacksTo(1).rarity(Rarity.RARE)
                .component(MIDatacomponet.MI_TAGS.get(),new MITags(List.of("streamer"), new ArrayList<>())));
    }
    ...
    public void appendHoverText(@NotNull ItemStack itemstack, @NotNull TooltipContext context, @NotNull List<Component> list, @NotNull TooltipFlag flag) {
        ...
        if (Screen.hasShiftDown()) {
            String[] description = Component.translatable("tooltip.mystias_izakaya.zheng_guo").getString().split("§n");
