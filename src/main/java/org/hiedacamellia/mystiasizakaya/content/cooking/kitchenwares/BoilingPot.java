...
public class BoilingPot {
    public static List<String> get(List<String> raws) {
        ...
        if (raws.contains("mystias_izakaya:mo_gu") && raws.contains("mystias_izakaya:xia") && raws.contains("mystias_izakaya:zhang_yu") && raws.contains("mystias_izakaya:la_jiao")) {
            rawlist.add("mystias_izakaya:mo_nv_de_wu_ta_hui");
        }
        if (raws.contains("mystias_izakaya:lu_rou") && raws.contains("mystias_izakaya:song_lu") && raws.contains("mystias_izakaya:chan_shui")) {
            rawlist.add("mystias_izakaya:ni_jiu_zi_yu");
        }
        if (raws.contains("mystias_izakaya:he_niu") && raws.contains("mystias_izakaya:ji_shang_jin_qiang_yu")
                && raws.contains("mystias_izakaya:hei_mao_zhu_rou") && raws.contains("mystias_izakaya:he_tun") && raws.contains("mystias_izakaya:song_lu")) {
            rawlist.add("mystias_izakaya:huan_xiang_fou_tiao_qiang");
        }
        if (raws.contains("mystias_izakaya:niu_rou")) {
            rawlist.add("mystias_izakaya:shi_zi_tou");
        }
        if (raws.contains("mystias_izakaya:niu_rou")
                && raws.contains("mystias_izakaya:luo_bu")
                && raws.contains("mystias_izakaya:dou_fu")
                && Collections.frequency(raws, "mystias_izakaya:la_jiao") == 2) {
            rawlist.add("mystias_izakaya:jue_jiao_guan_dou_zhu");
        }
        return rawlist;
    }
}
